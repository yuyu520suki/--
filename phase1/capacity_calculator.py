...
def generate_pm_curve(b: float, h: float, As_total: float, 
    ...
    def compute_nm(x):
        ...
        # 混凝土合力作用点 (距顶边距离)
        # 如果 h_eff < h (部分受压)，重心在 h_eff/2
        # 如果 h_eff = h (全受压)，重心在 h/2
        y_c_top = h_eff / 2
...
if __name__ == "__main__":
    print("=" * 60)
    print("承载力计算器测试 (GB 50010-2010) - Grok 重构版")
    print("=" * 60)
    ...
    # 理论验证
    N0_theory = (F_C * b_col * h_col + F_Y_PRIME * As_col) / 1000
    print(f"\n理论纯压承载力: {N0_theory:.1f} kN")
    print(f"计算纯压承载力: {pm_curve[0][0]:.1f} kN")
